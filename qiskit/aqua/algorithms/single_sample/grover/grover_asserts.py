...
class Grover(QuantumAlgorithm):
    ...
    def __init__(self, oracle, init_state=None, incremental=False, num_iterations=3, mct_mode='basic'):
        """
        Constructor.
        ...
        self._bp_amplitude_amplification = []
    ...
    def construct_circuit(self, measurement=False):
        ...
        if self._qc_amplitude_amplification is None:
            qreg = self._oracle.variable_register
        ...
        qc = QuantumCircuit(self._oracle.variable_register, self._oracle.output_register, measurement_cr)

        qc.u3(pi, 0, pi, self._oracle.output_register)  # x
        qc.u2(0, pi, self._oracle.output_register)  # h
        qc += self._init_state_circuit

        self._bp_amplitude_amplification = [ qc.copy(breakpoint.name).extend(breakpoint) for breakpoint in self._bp_amplitude_amplification ]
        ...
        return self._bp_amplitude_amplification, qc
    ...
    def _run(self):
        if self._incremental:
            ...
            def _try_current_max_num_iterations():
                ...
                qreg = self._oracle.variable_register
                creg = ClassicalRegister(len(qreg), name='m')
                self._qc_amplitude_amplification = QuantumCircuit(qreg, creg)
